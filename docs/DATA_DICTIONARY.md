...
| **Avg CLV** (ledger) | Mean closing-line value of the rule's bets that event: `taken_odds / closing_odds − 1` per bet (same-book Sportsbet basis both ends so the vig ~cancels; positive = beat the close), averaged. Closing odds come from `closing_odds.json`, a snapshot taken ~90 min before the card's first fight by `scripts/snapshot_closing_odds.py` (scheduled as a one-shot by the Friday card-day job — The Odds API drops events once they start, so the snapshot cannot be taken at scoring time). `-` for events before the CLV change (Sep 2026) or when no snapshot covered the bets. CLV grades the *price taken*, not the result, so it converges toward a rule's true edge far faster than win/loss does. |
